...
// number of tiles by default
var pix = 8;
createButton(1);
createButton(2);
createButton(3);
// tile with pixels by default
createTile(pix);
// applying painting method by default
paint3();
  ...
  $('.tile').on('mouseenter', function() {
      ...
      if (($(this).css('background-color')) === 'rgb(128, 128, 128)') {
        ...
        $(this).css('background-color', function() {
          ...
          // get current rgb background color
          var rgb = this.style.backgroundColor.split(/\(|\)/);
          // split to array
          rgb = rgb[1].split(',');
          // convert to hsl
          var toHsl = rgbToHsl(rgb[0], rgb[1], rgb[2]);
          $(this).css('background-color', function() {
...
function createButton(num) {
  ...
  $('#btn' + num).on('click', function() {
    var pix = prompt('How many pixels do you want?');
    // Create grid
    createTile(pix);
    // --- assigning paint method for button
    // calling a choosen paint method
    whichPaint['paint' + num]();
  });
}
// object to use paint function names as a variable
var whichPaint = {
  paint1: paint1,
  paint2: paint2,
  paint3: paint3
